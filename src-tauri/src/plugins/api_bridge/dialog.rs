...
pub fn inject_dialog_api<'js>(ctx: &Ctx<'js>, parent: &Object<'js>) -> Result<(), String> {
        ...
        move |_ctx: Ctx<'js>, _options: Value| -> Result<Value<'js>, rquickjs::Error> {
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                if result.is_empty() || result == "null" {
                    ...
                    Ok(Value::from_string(JsString::from_str(c, &result)?))
                }
            }
    ...
    let show_message_box_fn = Function::new(
        ctx.clone(),
        move |_ctx: Ctx<'js>, _options: Value| -> Result<Value<'js>, rquickjs::Error> {
            Ok(Value::from_string(JsString::from_str(_ctx, "OK")?))
        },
