...
**MVP 交付物**：7 个 QuickJS 插件 + 1 个 Webview 插件（共 8 个）

| 序号 | 插件名称 | 类型 | WASM 依赖 | 功能描述 |
|------|----------|------|-----------|----------|
| 1 | 快捷命令 | QuickJS | - | 用户自定义命令脚本 |
| 2 | 剪贴板增强 | QuickJS | - | 剪贴板历史、搜索 |
| 3 | 计算器 | QuickJS | - | 基础数学运算 |
| 4 | 二维码 | QuickJS | ✅ crypto | 生成/解析二维码 |
| 5 | 文件搜索 + 应用搜索 | QuickJS | - | 本地文件定位 + 应用快速启动 |
| 6 | 截图增强 | QuickJS | ✅ AI | 截图 + OCR + 标注 |
| 7 | 云端同步配置 | Webview | - | 配置跨设备同步 |
...
| 决策项 | 决策 | 来源 |
|--------|------|------|
| MVP 边界 | QuickJS 优先 + 统一架构设计 + WASM 包含 | 1.1 |
| 插件数量 | 8 个（7 QuickJS + 1 Webview） | 1.3 |
| 需求变更 | WASM 包含（crypto + AI） | 1.5 |
| API 兼容性 | B 方案 - 兼容 + 扩展 | 2.1 |
| QuickJS 集成 | 统一架构 + Tauri Event + window.utools | 2.2 |
| Webview 隔离 | 先 iframe 后多窗口 | 2.3 |
| WASM 技术选型 | wasm-pack + wasm-bindgen | 2.4 |
| 快捷键方案 | 可自定义 + 首次使用时检测 | 3.1 |
| Onboarding | 完整引导（4步）+ 3个核心插件 | 3.2 |
| 中文分词 | tiny-fuzzy 前端分词 | 3.4 |
| 主题系统 | CSS Custom Properties + 3个内置主题 | 3.5 |
| 插件分发 | GitHub Release + .zip + 拖拽安装 | 4.2 |
| 跨平台 | MVP 后再适配 macOS/Linux | 4.3 |
| AI 集成 | 暂不集成（保留 WASM AI 补丁） | 4.4 |
| 社区运营 | GitHub Discussions | 4.5 |
...
| 问题 | 决策 | 说明 |
|------|------|------|
| 兼容性实现 | **兼容 + 扩展** | 兼容 uTools API + Corelia 扩展 |
| 兼容范围 | **uTools 核心 API** | MVP 仅兼容 uTools |
| 扩展 API | **`window.corelia`** | WASM 补丁调用、云端同步等 |
...
| Webview 隔离 | **iframe（MVP）** | 后续可扩展多窗口 |
...
*最后更新: 2026-04-02*
*状态: Draft*
*更新内容: 新增决策记录(1.5)、API兼容性策略(2.2.1)、QuickJS与Webview统一架构(2.2.2)、用户体验决策(2.5.1-2.5.4)、生态与社区(3.1-3.5)、章节重新编号
